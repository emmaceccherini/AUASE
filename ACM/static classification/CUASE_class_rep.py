...
# 50% training, 40% testing  and 10% for validation at each time point 
# only use the labeled one for testing 
np.random.seed(0)
train_idx = {}
test_idx = {}
val_idx = {}
for key in labels_encoded.keys():
    ...
    train_idx[key] = perm[:int(0.5*n_k)]
    test_idx[key] = perm[int(0.5*n_k):int(0.9*n_k)]
    val_idx[key] = perm[int(0.9*n_k):]
#%%
import time
...
#%%
# save the mean accuracies
np.save('mean_accuracies_AUASE.npy', mean_accuracies)
np.save('mean_f1_AUASE.npy', mean_f1)