...
T = 10
n = 15851
embs = [np.load(f'../DySAT/default_embs_Epinions_{t}.npz')["data"]for t in range(T)] 
Y = np.vstack(embs)
Y = Y.reshape(T, n, d)   
#%%
...
for i in range(100):
    ...
    for t in range(T):
        ...
        # Create a list to hold training and testing indices176
        train_idx[key] = []
        test_idx[key] = []
...
np.save('mean_accuracies_DySAT.npy', mean_accuracies)
np.save('mean_f1_DySAT.npy', mean_f1)
#%%
mean_accuracies = np.mean(total_accuracies, axis=0)
mean_f1 = np.mean(total_f1, axis=0)
