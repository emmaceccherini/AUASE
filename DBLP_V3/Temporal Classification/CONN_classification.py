...
for a, alpha in enumerate(alphas):
    ...
    Y = np.vstack(embs)
    Y = Y.reshape(T, n, d) 
    X_train = np.vstack(Y[:4])
    X_train = X_train[active[:4].reshape(-1)]
    y_train = np.hstack(list(labels_encoded.values())[:4])
    y_train = y_train[active[:4].reshape(-1)]
...
Y =[]
for t in range(T):
    Y.append(torch.load(f'../CONN/embeddings_lp_DBLP_{t}_tw{alpha}.pth'))
#%%
...
X_test = np.vstack(Y[4:])
X_test = X_test[active[4:].reshape(-1)]
y_test = np.hstack(list(labels_encoded.values())[4:])
y_test = y_test[active[4:].reshape(-1)]
