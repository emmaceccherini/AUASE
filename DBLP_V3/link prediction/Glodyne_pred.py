...
d = 15
# load DBLP.npy
embeddings = np.load('../GloDyNE/embeddings/DBLP.npy', allow_pickle=True)    
Y= [] 
for i in range(T):
    Y.append(np.vstack(list(embeddings[i].values())))

# raise an error if the length of the embeddings is not equal to the number of time points
assert len(Y) == T
#%%
#%%
...
def get_samples(adj_matrices, active):
    ...
    for t in range(num_time_points - 1):
        ...
        # select the position of the nodes in the idx
        pos_pairs_t_idx =[]
        for pair in pos_pairs_t:
            # disregard the pair if the nodes are not in the idx
            if pair[0] not in idx[t] or pair[1] not in idx[t]:
                continue
            pair = (np.where(idx[t] == pair[0])[0][0], np.where(idx[t] == pair[1])[0][0])
            pos_pairs_t_idx.append(pair)

        neg_pairs_t_idx =[]
        for pair in neg_pairs_t:
            if pair[0] not in idx[t] or pair[1] not in idx[t]:
                continue        
            pair = (np.where(idx[t] == pair[0])[0][0], np.where(idx[t] == pair[1])[0][0])
            neg_pairs_t_idx.append(pair)

        pos_pairs.append(np.array(pos_pairs_t_idx).reshape(-1, 2))
        neg_pairs.append(np.array(neg_pairs_t_idx).reshape(-1, 2))
...
def get_samples(adj_matrices, active):
    ...
    for t in range(num_time_points - 1):
        ...
        while len(neg_pairs_t) < len(pos_pairs_t):  # Balance negatives to positives
            i, j = np.random.choice(active_nodes_t, 2, replace=False)
            if i not in idx[t] or j not in idx[t]:
                continue
            if adj_t[i, j] == 0:
                neg_pairs_t.add((i, j))
